        ...
        GJApi.openSession(function()
            {
                ...
                new FlxTimer().start(0.5, function(tmr:FlxTimer){pingSession();}, 0);
            });
    }
...
{    
    ...
    */
    public static var fontPath:String = "assets/fonts/seguibl.ttf";
    /**
    ...
    */
    ...
    public static var textArray:Array<String> = [
        "fnfever!!!!!!!",
        "friday night fever comes out in 35u423952349 years",
        "you 5'3 having ass",
        "[Funny IP address joke]",
        "penis!!",
        "wow!! such shorter dialogue!!!!",
        "login bitch",
        "isophoro her!e!!!",
        "rifxii is based",
        "isophoro is based"
...
class GameJoltLogin extends MusicBeatSubstate
{
    ...
    override function create()
    {
        ...
        var gamejolt:FlxSprite = new FlxSprite(0, 60);
        gamejolt.frames = Paths.getSparrowAtlas('gamejolt/gamejolt');
        gamejolt.animation.addByPrefix('idle', 'gamejolt logo', 24);
        gamejolt.animation.play('idle');
        add(gamejolt);
        gamejolt.screenCenter(X);
        FlxTween.tween(gamejolt, {y: gamejolt.y + 50}, {type: PINGPONG, ease:FlxEase.circOut});
        ...
        signInBox = new FlxButton(0, 430, "", function()
        {
            trace(usernameBox.text);
            trace(tokenBox.text);
            GameJoltAPI.authDaUser(usernameBox.text,tokenBox.text,true);
        });
        signInBox.loadGraphic(Paths.image('gamejolt/login'));
        signInBox.scrollFactor.set();


        helpBox = new FlxButton(0, 567, "", function()
        {
            if (!GameJoltAPI.getStatus())openLink('https://www.youtube.com/watch?v=T5-x7kAGGnE');
        });
        helpBox.loadGraphic(Paths.image('gamejolt/gamejolt token'));
        helpBox.scrollFactor.set();

        logOutBox = new FlxButton(0, 369.35, "", function()
        {
            GameJoltAPI.deAuthDaUser();
        });

        logOutBox.loadGraphic(Paths.image('gamejolt/log out'));
        logOutBox.scrollFactor.set();
        

        cancelBox = new FlxButton(870.9,620, "", function()
        {
            FlxG.save.flush();
        ...
        cancelBox.loadGraphic(Paths.image('gamejolt/notnow'));
        cancelBox.scrollFactor.set();
		...
		if(GameJoltAPI.getStatus() == true)
            {
                if(TitleState.lastState == true)
                    {
                        black = new FlxSprite(400,0).loadGraphic(Paths.image('halloweenBG', 'preload'));
    ...
    override function update(elapsed:Float)
    {
        ...
        if (GameJoltAPI.getStatus())
        {
            cancelBox.loadGraphic(Paths.image('gamejolt/continue'));
            cancelBox.x = helpBox.x;
            cancelBox.y = helpBox.y - 60;
            cancelBox.updateHitbox();
